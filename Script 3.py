"""
Script 3 — Daily food intake plots.

What it does
- Reads the raw CSV.
- Converts calendar dates to experimental days (Day 0 = 14 Aug by default).
- Uses daily food intake values for Days 1–11 (standard 24-hour intervals).
...
- Pooled plot (default): food_intake_daily_pooled_2024_2025.png/.pdf
- Pooled day-by-day summary: food_intake_daily_summary_pooled_2024_2025.csv
- Optional per-year plots: food_intake_daily_2024.png/.pdf, food_intake_daily_2025.png/.pdf
- Optional per-year summary (stacked): food_intake_daily_summary_by_year.csv
...
def summarize_daily(df: pd.DataFrame) -> pd.DataFrame:
    """Day-by-day summary per group."""
    s = (df.groupby(["conditions", "day"])["intake_g"]
...
def plot_daily(df: pd.DataFrame, out_png: Path, out_pdf: Path, vline_day: int = 6, dpi: int = 600,
    ...
    for cond in conditions:
        ...
        for _, dfr in dfg.groupby("bird_id"):
            ...
            ax.plot(dfr["day"].to_numpy(), dfr["intake_g"].to_numpy(),
                    linewidth=1.0, alpha=0.30, color=colors[cond])

        stats = summarize_daily(dfg)
    ...
    ax.set_ylabel("Daily food intake (g/day)")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Daily food intake plots (pooled and/or per year).")
    ...
    # keep only Days 1–11 for standard daily intervals
    ...
    if args.mode in {"pooled", "both"}:
        plot_daily(
            df,
            args.outdir / "food_intake_daily_pooled_2024_2025.png",
            args.outdir / "food_intake_daily_pooled_2024_2025.pdf",
            vline_day=args.vline_day,
            dpi=args.dpi,
        )
        summarize_daily(df).to_csv(
            args.outdir / "food_intake_daily_summary_pooled_2024_2025.csv",
            index=False
        )
    ...
    if args.mode in {"year", "both"}:
        ...
        for yr in sorted(df["year"].unique()):
            ...
            plot_daily(
                dfx,
                args.outdir / f"food_intake_daily_{yr}.png",
                args.outdir / f"food_intake_daily_{yr}.pdf",
                vline_day=args.vline_day,
                dpi=args.dpi,
            )
            s = summarize_daily(dfx)
            s.insert(0, "year", yr)
            yearly_rows.append(s)
        if yearly_rows:
            pd.concat(yearly_rows, ignore_index=True).to_csv(
                args.outdir / "food_intake_daily_summary_by_year.csv",
                index=False
            )
